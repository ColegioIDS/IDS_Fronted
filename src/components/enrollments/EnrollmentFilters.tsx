...
import { useEnrollment } from '@/hooks/useEnrollment'; // ✅ CAMBIO
import {
  Filter,
  X,
  RotateCcw,
...
const statusOptions = [
  ...
  { value: 'graduated', label: "Graduado", color: "bg-purple-100 text-purple-700" },
  { value: 'transferred', label: "Transferido", color: "bg-orange-100 text-orange-700" },
];
...
export default function EnrollmentFilters({ onFiltersChange }: EnrollmentFiltersProps) {
  ...
  // ✅ CAMBIO: Obtener datos de formData
 const cycles = [formData?.activeCycle].filter(Boolean) || [];
  const grades = formData?.grades || [];
  const isLoadingCycles = isLoadingFormData;
  ...
  const availableSections = selectedGrade
    ? grades.find(g => g.id.toString() === selectedGrade)?.sections || []
    : [];
  ...
  const applyCurrentCycleFilter = () => {
    const activeCycle = formData?.activeCycle;
    if (activeCycle) {
    ...
    <div className="space-y-6">
                ...
                <SelectItem key={status.value} value={status.value}>
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${
                      ...
                      status.value === 'graduated' ? 'bg-purple-500' : 'bg-orange-500'
                    }`} />
                    {status.label}
                  </div>
                </SelectItem>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    ...
                    disabled={isLoadingCycles || !formData?.activeCycle}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {selectedCycle && (
            <Badge variant="secondary" className="bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400">
              Ciclo: disabled={isLoadingCycles || !formData?.activeCycle}
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1">
        <div className="flex items-center gap-4">
          ...
          <span>Grados activos: {grades.length || 0}</span>
          <span>Secciones: {grades.reduce((acc, g) => acc + (g.sections?.length || 0), 0)}</span>
        </div>
      </div>
    </div>
  );
