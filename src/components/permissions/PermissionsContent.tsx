...
export default function PermissionsContent() {
  const [filters, setFilters] = useState<PermissionFilters>({
    page: 1,
    limit: 100, // Traer más para agrupar localmente
    sortBy: 'module',
    sortOrder: 'asc',
  });

  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 9; // Módulos por página
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-4 border border-gray-100 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <div className="text-sm text-gray-600 dark:text-gray-400">
                  ...
                  {Math.min(currentPage * itemsPerPage, filteredGroups.length)} de{' '}
                  {filteredGroups.length} módulos
                </div>
