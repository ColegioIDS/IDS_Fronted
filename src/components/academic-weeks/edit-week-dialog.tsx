// components/academic-weeks/edit-week-dialog.tsx
"use client";

import React, { useState, useEffect } from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { Edit2, Save, Loader2, AlertCircle, RefreshCw, CalendarIcon, BookOpen, PencilIcon, RotateCcw } from 'lucide-react';
...
export function EditWeekDialog({ week, open, onOpenChange }: EditWeekDialogProps) {
  const { updateWeek, isUpdating } = useAcademicWeekActions();
  ...
  const form = useForm<UpdateAcademicWeekFormValues>({
    resolver: zodResolver(updateAcademicWeekSchema),
  });
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (bimesterId) {
      ...
      setSelectedBimester(bimester || null);
      // Solo limpiar las fechas si el bimestre cambió del original
      if (bimesterId !== week.bimesterId) {
        form.setValue('startDate', '');
        form.setValue('endDate', '');
      }
    }
  }, [form.watch('bimesterId'), bimesters, form, week.bimesterId]);
  ...
  const isDateOccupied = (date: Date) => {
    ...
    // Obtener todas las semanas del bimestre seleccionado, excluyendo la semana actual
    const bimesterWeeks = weeks.filter(w => 
      w.bimesterId === selectedBimester.id && w.id !== week.id
    );
    
    // Verificar si la fecha está en el rango de alguna semana existente
    return bimesterWeeks.some(w => {
      const weekStart = new Date(w.startDate.split('T')[0] + 'T00:00:00');
      const weekEnd = new Date(w.endDate.split('T')[0] + 'T00:00:00');
