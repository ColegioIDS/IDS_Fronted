// src/components/ui/ModalWarningConfirm.tsx
"use client";
import React from "react";

interface ModalWarningConfirmProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => void;
  title?: string;
...
export function ModalWarningConfirm({
  isOpen,
  onClose,
  onConfirm,
  title = "Warning Alert!",
  ...
  cancelText = "Cancelar",
}: ModalWarningConfirmProps) {
  if (!isOpen) return null;

  return (
<div className="fixed inset-0 z-90001 flex items-center justify-center p-4 backdrop-blur-sm bg-black/10">

      <div className="max-w-[600px] w-full p-5 lg:p-10 bg-white dark:bg-gray-800 rounded-lg shadow-lg">
        <div className="text-center">
          ...
          <div className="flex items-center justify-center w-full gap-3 mt-7">
            <button
              type="button"
              onClick={onClose}
              className="flex justify-center w-full px-4 py-3 text-sm font-medium rounded-lg bg-gray-300 text-gray-800 hover:bg-gray-400 sm:w-auto"
            >
              {cancelText}
            </button>
            ...
            <button
              type="button"
              onClick={onConfirm}
              className="flex justify-center w-full px-4 py-3 text-sm font-medium rounded-lg bg-warning-500 text-white shadow-theme-xs hover:bg-warning-600 sm:w-auto"
            >
              {confirmText}
            </button>
          </div>
