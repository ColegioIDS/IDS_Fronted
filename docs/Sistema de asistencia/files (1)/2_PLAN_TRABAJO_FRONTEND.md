...
### **FASE 1: FOUNDATION - Tipos, Service, Hooks (4-5 horas)**

**Por qué primero:** Sin estos, no puedes desarrollar componentes.

#### 1.1 - Types (`attendance.types.ts`) - 1 hora
```
✓ Role interface
✓ AttendanceStatus interface
✓ StudentAttendance interface
✓ StudentClassAttendance interface
✓ StudentAttendanceReport interface
✓ ValidationChecks interface
✓ AttendanceFilters interface
✓ DailyRegistrationData interface
✓ BulkUpdateData interface
...
#### 1.2 - Schemas (`attendance.schema.ts`) - 1 hora
```
✓ CreateAttendanceSchema (Zod)
✓ UpdateAttendanceSchema (Zod)
✓ DailyRegistrationSchema (Zod)
✓ BulkUpdateSchema (Zod)
✓ FilterSchema (Zod)
✓ DateRangeSchema (Zod)
```
...
#### 1.3 - Service (`attendance.service.ts`) - 1.5 horas
```
...
- [ ] Errores lanzan excepciones correctas
- [ ] Tipos correctos en respuestas

---

#### 1.4 - Hooks (`hooks/data/attendance/`) - 1 hora
```
✓ useAttendance.ts (Hook principal)
✓ useAttendanceValidations.ts
✓ useAttendanceReport.ts
✓ useAttendanceFilters.ts
✓ useDailyRegistration.ts
```
